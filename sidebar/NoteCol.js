...
L.make.NoteCol = Backbone.Collection.extend({
   ...
   * @param {boolean} opt_firstLoad True if these notes are first notes loaded into sidebar.
   */
  addNotes_: function(notes, opt_firstLoad) {
	...
	note.meta = {};
      ...
      note.meta = ((note.meta instanceof String) ? JSON.parse(note.meta) : note.meta);
      note.id = note.jid;
      this.add(note);
  ...
  performFragmentedSearch_: function(searchTerms, notes, opt_visibleJIDs) {
	...
	for (var j = 0; j < searchTerms.length; j++) {
	  ...
	  var termMatches = contents.indexOf(term) !== -1; // - non-compatible with IE
	  if ((isPositiveTerm && !termMatches) || (!isPositiveTerm && termMatches)) {
	...
	this_.performFragmentedSearch_(searchTerms, contNotes, visibleJIDs);
      }, 40);
    } else {
